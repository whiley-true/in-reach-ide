...
class StatusBar(QWidget):
    def __init__(self, window: QWidget, parent: QWidget | None = None, *, edge_resize: bool = True) -> None:
        ...
        # The active env (see in_reach.app.script_preprocess), on the otherwise empty left edge --
        # clickable to change it, hidden with no project open or nothing to choose between.
        self.env_label = _ClickableLabel()
        self.env_label.hide()
    ...
    def set_env(self, name: str | None, *, on_click: Callable[[], None]) -> None:
        """Shows the left-edge env segment: ``Env: <name>``, or ``No env`` when the project has envs but
        none is chosen. Clicking it runs ``on_click`` (a quick-pick)."""
        self.env_label.setText(f"Env: {name}" if name else "No env")
        self.env_label.set_on_click(on_click)
        self.env_label.show()

    def clear_env(self) -> None:
        """Hides the env segment -- no project open, or it has no envs to pick from."""
        self.env_label.hide()
