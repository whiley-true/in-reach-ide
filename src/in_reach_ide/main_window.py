...
class MainWindow(QWidget):
    ...
    def apply_settings_changes(self) -> None:
        ...
        + ``edit/rvt/script.txt`` into a real gametype ``.bin`` (PROMPT.md: "applying changes
    ...
    def _sync_project_title(self, folder: Path) -> None:
        ...
        :func:`~in_reach.app.new_project.read_project_title` (the Explorer's own project tab, any
        already-open editor's breadcrumb, the Welcome tab's Recent list, and the File menu's Open
        Recent submenu, the last two both re-reading it fresh on their own next open regardless) --
        back in sync (PROMPT.md: "when a project name is changed [via rvt or via apply settings.json
        change] - the project title should change in the tabs and in the breadcrumb"; later: "it
        needs to be renamed in recents in dropdown and in the welcome window").
