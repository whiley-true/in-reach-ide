"""The "Select Env" command palette entry -- the way a user picks which
``script/env/<name>.env`` env the active project builds with (see
:mod:`in_reach.app.script_preprocess`), rather than hand-editing ``active_env.txt``."""
import json
...
def _project(tmp_path: Path, *envs: str) -> Path:
...
def _command(window: MainWindow):
    return next(c for c in window.build_command_palette_commands() if c.label == "Select Env")
...
def test_choosing_with_no_project_open_is_a_no_op(project_window) -> None:
    project_window._set_env("dev")  # must not raise
...
# -- the status bar's env indicator -------------------------------------------------------------------------


def _indicator(window: MainWindow):
    return window.status_bar.env_label
...
def test_editing_the_active_profile_pointer_and_saving_refreshes_the_indicator(project_window, tmp_path: Path, qtbot) -> None:
    ...
    script_preprocess.set_active_env(folder, "dev")
    project_window._on_project_opened(folder)
    pointer = folder / "script" / "env" / script_preprocess.ACTIVE_ENV_FILENAME
...
# -- stamping a release from an env that isn't the release one --------------------------------------------


def _stampable(project_window, tmp_path: Path, *envs: str) -> Path:
    folder = _project(tmp_path, *envs)
    project_window._on_project_opened(folder)
    return folder
...
def test_stamping_off_the_release_profile_asks_first_and_cancelling_stamps_nothing(project_window, tmp_path: Path, monkeypatch) -> None:
    ...
    script_preprocess.set_active_env(folder, "dev")
    asked, stamped = [], []
    monkeypatch.setattr(project_window, "_confirm_stamp_off_release_env", lambda active: asked.append(active) or False)
...
def test_stamping_anyway_stamps(project_window, tmp_path: Path, monkeypatch) -> None:
    folder = _stampable(project_window, tmp_path, "dev", "release")
    stamped = []
    monkeypatch.setattr(project_window, "_confirm_stamp_off_release_env", lambda active: True)
    monkeypatch.setattr("in_reach.app.vcs.stamp", lambda *a, **k: stamped.append((a, k)))

    project_window.vcs_stamp("first", "0.1.0")

    assert stamped == [((folder, "first"), {"version": "0.1.0"})]
    assert script_preprocess.active_env_name(folder) is None  # nothing was switched behind the user's back
...
def test_stamping_on_the_release_profile_does_not_ask(project_window, tmp_path: Path, monkeypatch) -> None:
    ...
    script_preprocess.set_active_env(folder, "release")
    stamped = []
    monkeypatch.setattr(project_window, "_confirm_stamp_off_release_env", lambda active: pytest.fail("asked"))
    monkeypatch.setattr("in_reach.app.vcs.stamp", lambda *a, **k: stamped.append(a))
...
@pytest.mark.parametrize("envs", [(), ("dev",), ("dev", "staging")])
def test_a_project_with_no_release_profile_is_never_asked(project_window, tmp_path: Path, monkeypatch, envs) -> None:
    _stampable(project_window, tmp_path, *envs)
    stamped = []
    monkeypatch.setattr(project_window, "_confirm_stamp_off_release_env", lambda active: pytest.fail("asked"))
    monkeypatch.setattr("in_reach.app.vcs.stamp", lambda *a, **k: stamped.append(a))
...
def test_the_warning_names_the_profile_in_use_or_says_none(project_window, monkeypatch) -> None:
    ...
    project_window._confirm_stamp_off_release_env("dev")
    project_window._confirm_stamp_off_release_env(None)

    assert texts == ["The active env is 'dev', not 'release'.", "No env is active, not 'release'."]
